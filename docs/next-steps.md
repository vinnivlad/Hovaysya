...
**Що працює просто зараз.** `python -m tools.live.run` слухає пʼять каналів,
приймає рішення тією ж політикою, що й оцінка, і шле їх у приватний
телеграм-канал через бота. Затримка виявлення виміряна: медіана 6 с, p90 9 с.
Запускається з `D:\Work\Hovaysya`, дані там же в `data/`.
...
- Oracle: акаунт створюється, регіон Марсель. Форма — 1 OCPU / 1 ГБ, не 6.
- Крюківщина: одна побудка проти однієї «далеко», третій випадок вирішить.
